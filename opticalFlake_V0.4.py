...
def get_line_rgb_values(
    image: Image.Image, x1: int, y1: int, x2: int, y2: int
) -> tuple:
    """
...
def calculate_contrast(
    ...
    for (x1, y1), (x2, y2) in segments:
        ...
        red_contrast = (
            (bg_r - avg_red) / (bg_r + avg_red) if bg_r > 0 else np.zeros_like(avg_red)
        )
        green_contrast = (
            (bg_g - avg_green) / (bg_g + avg_green)
            if bg_g > 0
            else np.zeros_like(avg_green)
        )
        blue_contrast = (
            (bg_b - avg_blue) / (bg_b + avg_blue)
            if bg_b > 0
            else np.zeros_like(avg_blue)
        )
...
class ScreenCaptureOverlay(QWidget):
    ...
    def __init__(self):
        ...
        self.first_click_set = (
            False  # True if first corner is set (click-then-click mode)
        )
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            if not self.first_click_set:
                ...
                self.is_dragging = (
                    False  # Will be set True in mouseMoveEvent if dragging
                )
            else:
...
class ImageCanvas(QGraphicsView):
    ...
    drawing_mode_changed = Signal(
        bool
    )  # Emits True when entering drawing mode, False when exiting
    ...
    def __init__(self):
        ...
        self.bg_preview_line: Optional[QGraphicsLineItem] = (
            None  # Preview line for polygon mode
        )
    ...
    def start_linecut_mode(self) -> bool:
        ...
        self.current_linecut_color = MEASUREMENT_COLORS[
            self.measurement_count % len(MEASUREMENT_COLORS)
        ]
        self.setCursor(Qt.CursorShape.CrossCursor)
        self.drawing_mode_changed.emit(True)
        return True
    ...
    def _draw_background_preview(self, mouse_pos: QPointF):
        ...
        if self.bg_is_dragging and self.bg_drag_start:
            ...
            self.bg_preview_rect.setPen(
                QPen(QColor(0, 255, 0), 2, Qt.PenStyle.DashLine)
            )
            ...
            self.bg_preview_line = QGraphicsLineItem(
                last_point.x(), last_point.y(), mouse_pos.x(), mouse_pos.y()
            )
            self.bg_preview_line.setPen(
    ...
    def _draw_linecut_preview(self, mouse_pos: Optional[QPointF] = None):
        ...
        if len(self.linecut_points) > 0:
            ...
            ellipse = QGraphicsEllipseItem(
                start_point.x() - 2.5, start_point.y() - 2.5, 5, 5
            )
            ellipse.setPen(pen)
        ...
        for i in range(len(self.linecut_points) - 1):
            ...
            arrow = self._create_arrowhead(
                p1.x(), p1.y(), p2.x(), p2.y(), linecut_color
            )
            if arrow:
    ...
    def _create_arrowhead(
        self, x1: float, y1: float, x2: float, y2: float, color: QColor, size: float = 8
        ...
        right = QPointF(
            base_center_x - px * size * 0.5, base_center_y - py * size * 0.5
        )
    ...
    def _draw_width_preview(
        self, p1: QPointF, p2: QPointF, pen: QPen, preview: bool = False
    ):
    ...
    def _finalize_linecut(self):
        ...
        if len(self.linecut_points) >= 2:
            ...
            self._add_persistent_linecut(
                self.linecut_points, self.current_linecut_color
            )
    ...
    def update_persistent_linecut_width(
        self, index: int, new_width: int, segments: list, color: str
    ):
        ...
        if 0 <= index < len(self.persistent_linecut_items):
            ...
            for i, ((x1, y1), (x2, y2)) in enumerate(segments):
                ...
                # Draw width indicators
                if half_width >= 1:
                    nx1, ny1, nx2, ny2 = offset_parallel_line(
    ...
    def display_rgb_text(self, rgb: tuple):
        ...
        if self.pixmap_item:
            ...
            self.rgb_text_item.setPos(
                img_rect.right() - text_width - 5, img_rect.top() + 5
            )
        else:
    ...
    def mousePressEvent(self, event):
        ...
        if self.drawing_mode == "background":
            if len(self.polygon_points) == 0:
                ...
                self.bg_is_dragging = (
                    False  # Will be set to True in mouseMoveEvent if dragging
                )
            else:
...
class DataDisplayPanel(QWidget):
    ...
    def __init__(self):
        ...
        self.red_checkbox.setStyleSheet(
            "QCheckBox { color: #cc0000; font-weight: bold; }"
        )
        ...
        self.green_checkbox.setStyleSheet(
            "QCheckBox { color: #008800; font-weight: bold; }"
        )
        ...
        self.blue_checkbox.setStyleSheet(
            "QCheckBox { color: #0066cc; font-weight: bold; }"
        )
        ...
        self.ref_red_spinbox.setSingleStep(0.15)
        self.ref_red_spinbox.setValue(self.reference_values["red"])
        self.ref_red_spinbox.setStyleSheet("QDoubleSpinBox { color: #cc0000; }")
        self.ref_red_spinbox.valueChanged.connect(
            lambda v: self._on_ref_value_changed("red", v)
        )
        ...
        self.ref_green_spinbox.valueChanged.connect(
            lambda v: self._on_ref_value_changed("green", v)
        )
        ...
        self.ref_blue_spinbox.valueChanged.connect(
            lambda v: self._on_ref_value_changed("blue", v)
        )
    ...
    def update_measurement_data(
        self, index: int, red: np.ndarray, green: np.ndarray, blue: np.ndarray
    ):
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        capture_spacer.setFixedWidth(
            2
        )  # <-- TUNE THIS: pixels of space before Draw Background
        ...
        background_spacer.setFixedWidth(
            4
        )  # <-- TUNE THIS: pixels of space before Draw Background
        ...
        if is_drawing:
            ...
            if tab and tab.canvas.drawing_mode == "background":
                self.mode_indicator.setText(
                    "🎯 Drag for rectangle, or click vertices then double-click to finish polygon"
                )
            elif tab and tab.canvas.drawing_mode == "linecut":
                self.mode_indicator.setText(
                    "🎯 Click to draw linecut, double-click to finish"
                )
        else:
